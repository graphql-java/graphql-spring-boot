...
public class GraphQLController {
    ...
    @PostMapping(value = "${graphql.url:graphql}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<Object> postByJson(@RequestBody Mono<GraphQLRequest> requestBodyMono,
                                   ServerWebExchange serverWebExchange) {
        return requestBodyMono.flatMap(request -> {
    ...
    @PostMapping(value = "${graphql.url:graphql}", consumes = {"application/graphql", "application/graphql; charset=utf-8"}, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<Object> post(
            @RequestBody Mono<String> requestBodyMono,
            ServerWebExchange serverWebExchange) {
        // * If the "application/graphql" Content-Type header is present,
        //   treat the HTTP POST body contents as the GraphQL query string.
        return requestBodyMono.flatMap(query -> executeRequest(query, null, Collections.emptyMap(), serverWebExchange));
    }

    @PostMapping(value = "${graphql.url:graphql}", consumes = MediaType.ALL_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
